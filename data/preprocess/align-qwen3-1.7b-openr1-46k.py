...
if __name__ == "__main__":
    ...
    parser.add_argument("--rollout_file", default="/mnt/weka/home/yongxin.wang/workspace/lark/DeepInformal/checkpoints/infer/Qwen3-1.7B-Openr1-46k/valid_generations/0.jsonl")
    ...
    # 保存为parquet文件
    output_file = os.path.join(output_dir, "qwen3-1.7b-openr1-46k.parquet")
    df.to_parquet(output_file, index=False)
    print(f"数据集已成功保存到: {output_file}")
    print(f"总共处理了 {len(filtered_data)} 条数据")


    # 选取128条数据，保存为qwen3-1.7b-openr1-46k-val.parquet
    ...
    # 保存为parquet文件
    output_file = os.path.join(output_dir, "qwen3-1.7b-openr1-46k-val.parquet")
    df.to_parquet(output_file, index=False)
