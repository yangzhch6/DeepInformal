...
def main():
    ...
    pipeline.run_pipeline(
        parquet_path="Jiahao004/DeepTheorem", # Jiahao004/DeepTheorem  yangzhch6/Putnam-Informal-1995-2024
        model_name="deepseek-chat",
        data_dir="/mnt/weka/home/yongxin.wang/workspace/lark/DeepInformal/gen_deepseek/deeptheorem-prompt3-deepseekv3chat",  # 数据保存目录
        k=1,  # 每个样本生成k个响应
        max_tokens=8192,  # 设置请求超时时间
        temperature=0.6,  # 设置temperature参数
        max_samples=130000,  # 限制处理样本数量（可选）
        parallel_num=512,   # 同时处理的样本数量
        question_key="informal_theorem_qa",  # 指定问题字段
        ground_truth_key="proof",  # 指定答案字段
        fail_regen=False  # 重新生成所有响应都不是"pass"的样本
    )
